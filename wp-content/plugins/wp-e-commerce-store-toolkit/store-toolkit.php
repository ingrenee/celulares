...
if( is_admin() ) {
	...
	function wpsc_st_add_settings_link( $links, $file ) {
		...
		if( $file == $this_plugin ) {
			// Settings
			$settings_link = sprintf( '<a href="%s">' . __( 'Settings', 'wpsc_st' ) . '</a>', add_query_arg( 'page', 'wpsc_st', 'options-general.php' ) );
			array_unshift( $links, $settings_link );
			if( function_exists( 'wpsc_find_purchlog_status_name' ) ) {
				// Manage
				$manage_link = sprintf( '<a href="%s">' . __( 'Manage', 'wpsc_st' ) . '</a>', add_query_arg( array( 'post_type' => 'wpsc-product', 'page' => 'wpsc_st-toolkit' ), 'edit.php' ) );
				array_unshift( $links, $manage_link );
			} else {
				// Uninstall
				$uninstall_link = sprintf( '<a href="%s#uninstall-wpecommerce">' . __( 'Uninstall', 'wpsc_st' ) . '</a>', add_query_arg( 'page', 'wpsc_st', 'options-general.php' ) );
