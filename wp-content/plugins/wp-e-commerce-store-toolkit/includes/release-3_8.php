...
';
	...
	function wpsc_st_manage_purchase_logs_actions_button( $default = null, $column_name = null, $item = null ) {
		...
		if( $column_name == 'actions' ) {
			if( isset( $item->id ) && $item->id ) {
				...
				// Re-send
				$output .= sprintf( '<a href="%s" class="button" onclick="%s"><abbr title="' . __( 'Re-send Sale', 'wpsc_st' ) . '">' . __( 'Resend', 'wpsc_st' ) . '</abbr></a>', add_query_arg( array( 'c' => 'item_details', 'id' => $item->id, 'email_buyer_id' => $item->id ) ), "if( confirm( '" . esc_js( sprintf( __( "Are you sure you want to resend the receipt for this Sale (#%s)?", "wpsc" ), $item->id ) ) . "') ) { return true; } return false;" );
			}
		}
		return $output;
	...
	function wpsc_st_product_columns( $columns = array() ) {
		...
		$columns['stock_ind'] = __( 'Stock', 'wpsc_st' );
		return $columns;
...
/*
	function wpsc_st_manage_edit_products_column( $columns ) {
		...
		$columns['custom'] = '<span>' . __( 'Custom', 'wpsc_st' ) . '</span>';
		return $columns;
...
*/
	...
	function wpsc_st_products_meta_boxes() {
		...
		add_meta_box( 'wpsc_st_product_data_meta_box', __( 'Product Post Meta', 'wpsc_st' ), 'wpsc_st_product_data_meta_box', $pagename, 'normal', 'default' );
...
*/
	...
	function wpsc_st_wpsc_admin_quantity_html_page() {
		...
		$low_stock = get_option( 'wpsc_low_stock', 3 ); ?>
<h3 class="form_group"><?php _e( 'Stock Indicator Settings', 'wpsc_st' ); ?></h3>
<table class="wpsc_options form-table">

	<tr>
		<th><strong><?php _e( 'Low Stock', 'wpsc_st' ); ?></strong></th>
		<td>
			<input type="text" name="wpsc_options[wpsc_low_stock]" value="<?php echo $low_stock; ?>" size="3" maxlength="3" />
			<p class="description"><?php _e( 'Adjust the low stock amount which triggers the \'Low stock\' notice on the Manage Products screen.', 'wpsc_st' ); ?></p>
		</td>
	</tr>
